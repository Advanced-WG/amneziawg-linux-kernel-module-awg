...
static int bogus_endpoints = 0;
static char bogus_endpoints_prefix[64] = "127.0.0.0/8";
static char bogus_endpoints_prefix6[64] = "ff80::/16";
...
/* State machine positions for resumable device attribute dump.
 * When the netlink skb runs out of space, we remember which attribute
 * we were about to emit and resume from there on the next callback.
 */
enum wg_dump_device_pos {
	WG_DUMP_LISTEN_PORT = 0,
	WG_DUMP_FWMARK,
	WG_DUMP_IFINDEX,
	WG_DUMP_IFNAME,
	WG_DUMP_JC,
...
struct dump_ctx {
	...
	u8 device_pos;
	/* Snapshot of bogus_endpoints config, taken once in start() */
	int bogus_ep;
	struct ipv4_prefix bogus_prefix4;
	struct ipv6_prefix bogus_prefix6;
};

#define DUMP_CTX(cb) ((struct dump_ctx *)((cb)->args[0]))
...
get_peer(struct wg_peer *peer, struct sk_buff *skb, struct dump_ctx *ctx)
{
	...
	if (!allowedips_node) {
		...
		if (peer->endpoint.addr.sa_family == AF_INET) {
			...
			if (ctx->bogus_ep) {
				ret = generate_ipv4_address_with_prefix(&ctx->bogus_prefix4, &addr4.sin_addr.s_addr);
				if (ret < 0)
					goto err;
			}
			...
			if (ctx->bogus_ep) {
				ret = generate_ipv6_address_with_prefix(&ctx->bogus_prefix6, addr6.sin6_addr.s6_addr);
				if (ret < 0)
					goto err;
			}
...
/* Emit device-level attributes one at a time so that the dump can be
 * resumed across multiple netlink messages if the skb runs out of space.
 * Returns 0 on success, -EMSGSIZE if the current attribute did not fit.
 */
	...
	for (;;) {
		...
		case WG_DUMP_FWMARK:
			if (nla_put_u32(skb, WGDEVICE_A_FWMARK, wg->fwmark))
				return -EMSGSIZE;
			break;
		case WG_DUMP_IFINDEX:
			if (nla_put_u32(skb, WGDEVICE_A_IFINDEX,
					wg->dev->ifindex))
				return -EMSGSIZE;
			break;
		case WG_DUMP_IFNAME:
			if (nla_put_string(skb, WGDEVICE_A_IFNAME,
					   wg->dev->name))
				return -EMSGSIZE;
			break;
		case WG_DUMP_JC:
		...
		case WG_DUMP_I1:
			if (wg->ispecs[0].desc &&
			    nla_put_string(skb, WGDEVICE_A_I1,
					   wg->ispecs[0].desc))
				return -EMSGSIZE;
			break;
		case WG_DUMP_I2:
			if (wg->ispecs[1].desc &&
			    nla_put_string(skb, WGDEVICE_A_I2,
					   wg->ispecs[1].desc))
				return -EMSGSIZE;
			break;
		case WG_DUMP_I3:
			if (wg->ispecs[2].desc &&
			    nla_put_string(skb, WGDEVICE_A_I3,
					   wg->ispecs[2].desc))
				return -EMSGSIZE;
			break;
		case WG_DUMP_I4:
			if (wg->ispecs[3].desc &&
			    nla_put_string(skb, WGDEVICE_A_I4,
					   wg->ispecs[3].desc))
				return -EMSGSIZE;
			break;
		case WG_DUMP_I5:
			if (wg->ispecs[4].desc &&
			    nla_put_string(skb, WGDEVICE_A_I5,
					   wg->ispecs[4].desc))
				return -EMSGSIZE;
			break;
		case WG_DUMP_KEYS:
			/* Keep private/public key pair atomic: never split
			 * across multipart netlink chunks.
			 */
			down_read(&wg->static_identity.lock);
			if (wg->static_identity.has_identity) {
				size_t keys_size =
					2 * (nla_total_size(NOISE_PUBLIC_KEY_LEN));
				if (skb_tailroom(skb) < (int)keys_size) {
...
static int wg_get_device_start(struct netlink_callback *cb)
{
	...
	cb->args[0] = (long)ctx;
	ctx->wg = wg;
	ctx->device_pos = WG_DUMP_LISTEN_PORT;

	/* Snapshot bogus_endpoints config once for the entire dump */
	ctx->bogus_ep = bogus_endpoints;
	if (ctx->bogus_ep) {
		if (parse_ipv4_prefix(bogus_endpoints_prefix, &ctx->bogus_prefix4) < 0 ||
		    parse_ipv6_prefix(bogus_endpoints_prefix6, &ctx->bogus_prefix6) < 0) {
			ctx->bogus_ep = 0;
			net_dbg_ratelimited("amneziawg: invalid bogus_endpoints prefix, disabled for this dump\n");
		}
	}
...
static int wg_get_device_dump(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	if (ctx->device_pos < WG_DUMP_DEVICE_DONE) {
		unsigned int prev_len = skb->len;

		ret = wg_put_device_attrs(wg, skb, ctx);
		if (ret) {
			if (ret != -EMSGSIZE)
				goto out;
			/* Some attrs were written — send partial, resume later */
			if (skb->len > prev_len) {
				ret = 0;
				done = false;
				goto out;
			}
			/* Nothing fit at all — genuine overflow */
			goto out;
		}
	}
	...
	if (!peers_nest) {
		/* If device attrs were emitted but peers header doesn't fit,
		 * send what we have and resume with peers on next callback.
		 */
		if (ctx->device_pos >= WG_DUMP_DEVICE_DONE) {
			ret = 0;
			done = false;
